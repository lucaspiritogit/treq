...
$BinDir = if ($env:TREQ_BIN_DIR) { $env:TREQ_BIN_DIR } else { Join-Path $env:USERPROFILE ".local\bin" }
$BinName = "treq"
...
$Version = if ($env:TREQ_VERSION) { $env:TREQ_VERSION } else { "" }
if (-not $Version) {
  try {
    $latest = Invoke-RestMethod "https://api.github.com/repos/$Repo/releases/latest"
    $Version = [string]$latest.tag_name
  } catch {
    Fail "Could not resolve treq version from GitHub releases."
  }
}
...
$tmpRoot = Join-Path $env:TEMP ("treq-install-" + [Guid]::NewGuid().ToString("N"))
...
Write-Host ""
Write-Host "If '$BinDir' is not on PATH, add it and open a new shell."
